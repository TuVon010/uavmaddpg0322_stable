    ...
    def sample(self, N, to_gpu=False, norm_rews=True):
        ...
        if norm_rews:
            ...
            for i in range(self.num_agents):
                raw = self.rew_buffs[i][inds]
                rew_std = self.rew_buffs[i][:self.filled_i].std()
                rew_mean = self.rew_buffs[i][:self.filled_i].mean()
                if rew_std < 1e-6:
                    # Avoid division by zero when all rewards are similar
                    ret_rews.append(cast(raw - rew_mean))
                else:
                    ret_rews.append(cast((raw - rew_mean) / rew_std))
        else:
